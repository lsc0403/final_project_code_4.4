...
# Function for analyzing movie evaluation data
def analyze_reviews(data):
    ...
    for movie, reviews in data.items():
        ...
        positive_reviews_percentage = sum(1 for review in reviews if review['Predicted Class'] == 0) / total_reviews * 100
        negative_reviews_percentage = sum(1 for review in reviews if review['Predicted Class'] == 2) / total_reviews * 100
        average_review_length = sum(
            len(str(review['Review']).split()) for review in reviews) / total_reviews if total_reviews else 0

        # Calculate the year with the most reviews received
        ...
        # Determine the quality of movie reviews
        if positive_reviews_percentage >= 90:
...
# Generate HTML file function, including page layout improvements
...
if __name__ == "__main__":
    ...
    # Generate English descriptions based on analysis results
    descriptions = {}
    for movie, info in movie_reviews_analysis.items():
        desc = (f"This is a {info['review_quality']} movie, with {info['total_reviews']} reviews collected, "
                ...
                f"and with {info['negative_reviews_percentage']:.2f}% of people giving the movie a negative review, "

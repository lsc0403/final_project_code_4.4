...
# Modify the loss function to increase the weight of neutral and negative emotions
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(data_loader, desc="evaluating..."):
            ...
            for class_id, recall in enumerate(recall_per_class):
                ...
                # Collect all predictions and labels for later calculation of confusion matrix
...
metrics = {"train_losses": [], "train_accs": [], "train_recalls": [], "valid_losses": [], "valid_accs": [],
           "valid_recalls": []}
# Dynamically add recall keys for each class
...
for epoch in range(n_epochs):
    ...
    # pause
    if epoch < n_epochs - 1:  # If it's not the last epoch, pause it
...
# Draw loss curves for training and validation
fig = plt.figure(figsize=(10, 6))
ax = fig.add_subplot(1, 1, 1)
...
# Using sentiment prediction functions for prediction

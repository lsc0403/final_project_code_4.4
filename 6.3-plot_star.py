...
def generate_plot(average_ratings, review_counts, overall_average_ratings, total_reviews_by_movie):
    fig = go.Figure(layout=dict(height=1000))  # Set the chart height to 1000 pixels

    # Traces for monthly data
    for movie, ratings in sorted(average_ratings.items()):
        ...
        # Add total review count and overall average star rating next to the movie name
        movie_label_with_reviews = f"{movie} (Monthly) ({total_reviews_by_movie[movie]} reviews, {overall_average_ratings[movie]:.1f} stars)"
        trace = go.Scatter(x=months, y=avg_ratings, mode='lines+markers', name=movie_label_with_reviews,
                           text=hover_texts,
                           hoverinfo='text+name', visible='legendonly')
        fig.add_trace(trace)

    # Traces for yearly data (recalculated)
    for movie, ratings in sorted(average_ratings.items()):
        ...
        trace_yearly = go.Scatter(x=years, y=avg_ratings_yearly, mode='lines+markers',
                                  name=movie_label_with_reviews_yearly,
                                  text=hover_texts_yearly, hoverinfo='text+name', visible='legendonly')
        fig.add_trace(trace_yearly)

    # Add dropdown menus to switch views
    fig.update_layout(
        updatemenus=[
            dict(
                buttons=[
                ...
                pad={"r": 10, "t": 10},
                showactive=True,
                x=1.1,  # Adjust position to avoid overlapping with the title
                xanchor="right",
                y=1.15,  # Located outside above the chart
                yanchor="top"
            ),
